...
Prompt 要求：
- assetPrompts.promptBundles 必须与 storyboard.shots 一一对应，总数等于 shot 总数。
- 每个 promptBundle 必须包含 versionType、shotNumber、shotId、imagePrompt、videoPrompt、negativePrompt、styleLock、aspectRatio、usageWarning。
- imagePrompts 与 videoPrompts 也必须分别覆盖每一个 shot，sceneRef 与 shot.id 一一对应。
